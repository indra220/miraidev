...
export default function UserDashboardLayout({ children }: UserDashboardLayoutProps) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (session) {
        ...
        // Konversi session.user ke UserData untuk menghindari konflik tipe
        setUser({
          id: session.user.id,
  ...
  if (isAuthenticated === false) {
    return null; // Komponen akan redirect ke login
  }

  const menuItems = [
    { href: "/user/dashboard", label: "Dashboard", icon: Dashboard },
    { href: "/user/profile", label: "Profil", icon: User },
    { href: "/user/projects", label: "Proyek Saya", icon: FolderOpen },
    { href: "/user/settings", label: "Pengaturan", icon: Settings },
  ];
